 ...
 * @author ArcoSoft-Pc1
 */
@Named(value = "administradorasMB")
@SessionScoped
public class AdministradorasMB extends Controlador implements Serializable {
    ...
    private LoginMB aplicacionGeneralMB;

    //---------------------------------------------------
    //------------- FUNCIONES INICIALES  ----------------
    //---------------------------------------------------      
    public AdministradorasMB() {
        aplicacionGeneralMB = FacesContext.getCurrentInstance().getApplication().evaluateExpressionGet(FacesContext.getCurrentInstance(), "#{loginMB}", LoginMB.class);
    }
    ...
    public void confirmarEliminarAdministradora() {
        ...
        try {
            ...
            listaAdministradoras = administradoraFacade.buscarOrdenado();
            limpiarFormularioAdministradoras();
            aplicacionGeneralMB.cargarClasificacion(ClasificacionesEnum.Administradoras.Administradoras);
            RequestContext.getCurrentInstance().update("IdFormAdministradoras");
