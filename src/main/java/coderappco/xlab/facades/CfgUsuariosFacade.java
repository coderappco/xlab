...
public class CfgUsuariosFacade extends AbstractFacade<CfgUsuarios> {
    ...
    public List<CfgUsuarios> getUsuariosPRestadores(int idPrestador){
        try {
            Query q =  getEntityManager().createQuery("SELECT u FROM CfgUsuarios u WHERE u.tipoUsuario.id= :id");
            q.setParameter("id", idPrestador);
